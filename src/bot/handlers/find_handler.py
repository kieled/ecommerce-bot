...
async def search_input(update: Update, _: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.edit_text(
        localization.find_product_message,
        reply_markup=back_main_markup
    )
    return ID
...
async def invalid_product_id(update: Update, _: ContextTypes.DEFAULT_TYPE):
    try:
        await update.effective_message.reply_text(
            localization.invalid_id_message,
...
async def check_address(
    ...
    context.user_data['product_id'] = int(update.effective_message.text) \
        if not product_id \
        else product_id
    ...
    if len(addresses):
        await update.effective_message.reply_text(
            localization.founded_addresses_message,
            reply_markup=addresses_markup(addresses)
        )
...
async def buy_product(
    ...
    if not product:
        await update.effective_message.reply_text(
            localization.product_not_found_message,
            reply_markup=back_main_markup
        )
    else:
