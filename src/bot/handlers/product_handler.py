...
from telegram.ext import ConversationHandler, ContextTypes, MessageHandler, \
...
async def product_payment(update: Update, _: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(
        localization.send_check_message,
...
async def check_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with session() as s:
        customer_id: int = (await services.CustomerService(s).get_customer(update.effective_chat.id)).id

        transaction_id = await services.TransactionService(s).create(schemas.CreateTransactionSchema(
    ...
    check = io.BytesIO()
    await (await context.bot.get_file(
    ...
    async with session() as s:
        await services.TransactionService(s).add_check(schemas.AddCheckTransactionSchema(
            transaction_id=transaction_id,
            check_path=check_path
        ))
        admins = await services.AdminService(s).get_tg()
        order_id: int = await services.OrderService(s).create(schemas.CreateOrderSchema(
    ...
    await update.effective_message.reply_text(
        localization.product_complete_message(order_id),
        reply_markup=markups.back_main_markup
    )

    for admin in admins:
        await context.bot.send_message(
            admin,
            localization.new_order_message(
                order_id,
                data.get('price')
            )
        )
...
product_handler = ConversationHandler(
    ...
    states={
        PAYMENT: [
            CallbackQueryHandler(product_payment, callbacks.paid)
        ],
        CHECK: [
